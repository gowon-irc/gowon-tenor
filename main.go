...
type Options struct {
	...
	ApiKey string `short:"k" long:"api-key" env:"GOWON_TENOR_API_KEY" required:"true" description:"tenor api key"`
}
...
func genTenorHandler(apiKey string) func(m gowon.Message) (string, error) {
	return func(m gowon.Message) (string, error) {
		out, err := tenor(m.Args, apiKey)
...
func main() {
	...
	mr.AddCommand("gif", genTenorHandler(opts.ApiKey))
